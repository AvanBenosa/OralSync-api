...
namespace DMD.APPLICATION.PatientsModule.PatientEmergencyContact.Commands.Delete
{
    ...
    public class ComandHandler : IRequestHandler<Command, Response>
    {
        ...
        public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                var item = await dbContext.PatientEmergencyContacts
                    .FirstOrDefaultAsync(x => x.Id == request.Id);

                if (item == null) return new BadRequestResponse("Item may have been modified or removed.");

                dbContext.PatientEmergencyContacts.Remove(item);
