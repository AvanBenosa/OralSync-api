...
namespace DMD.APPLICATION.Finances.DentalInventories.Commands.Update
{
    [JsonSchema("UpdateDentalInventoryCommand")]
    ...
    public class CommandHandler : IRequestHandler<Command, Response>
    {
        ...
        public async Task<Response> Handle(Command request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                item.ItemCode = Normalize(request.ItemCode);
                item.Name = Normalize(request.Name);
                ...
                item.UnitOfMeasure = Normalize(request.UnitOfMeasure);
                item.UnitCost = request.UnitCost;
                item.SellingPrice = request.SellingPrice;
                item.TotalValue = request.TotalValue;
                item.SupplierName = Normalize(request.SupplierName);
                item.SupplierContactNumber = Normalize(request.SupplierContactNumber);
                item.SupplierEmail = Normalize(request.SupplierEmail);
                item.BatchNumber = Normalize(request.BatchNumber);
                item.ManufacturingDate = request.ManufacturingDate?.Date;
                item.ExpirationDate = request.ExpirationDate?.Date;
                item.LastRestockedDate = request.LastRestockedDate?.Date;
                item.LastUsedDate = request.LastUsedDate?.Date;
                item.UsageCount = request.UsageCount;
